	...
	iframe.src  = chrome.extension.getURL ("amazon.html");
...
//yourDIV.appendChild(iframe);
    ...
    var link='http://65.111.164.36/valueTag/appWebService/getcoupon.php?store=amazon-us';
var dispar=[];
  ...
  var x;var lstr='';
   ...
   // mydiv.appendChild(myimg);
    ...
    var mytb=document.getElementById("wrapper-new-gc");
...
//document.body.appendChild('<a href="#openModal">View Coupons! </a><div id="openModal" class="modalDialog"><div><a href="#close" title="Close" class="close">X</a><h2>Coupon Page</h2><div id="hello"> This page Displays coupons.</div></div></div>');
 ...
 // var initial=parseFloat(document.body.querySelectorAll('.summaryValue')[7].textContent.split('$')[1]);
   ...
   function apply(dispar,n)
    {
      var ip=document.getElementById('gcpromoinput');
      var bc=document.getElementById("button-add-gcpromo");

      if(n>=1)
      {
        if(document.getElementById('gcpromoerrorblock')!=null) 
        {
          money[n-1]=0;
    ...
    mya.onclick=function(){
        ...
        var i;
        if(dispar.length==0)  alert('No COUPONS PRESENT FOR THE SITE!');
 else apply(dispar,0);
    };
